...
NUM_ESTRELAS = 2
NUM_PLANETAS = 10000
RAIO_PROPORCIONALIDADE = 3  # Constante de ajuste para o raio

# Espaço virtual
ESCALA = 500  # Fator de escala: o mundo será 10x maior que a janela
...
def gerar_posicao_aleatoria():
    x = random.uniform(40000, 120000)
    y = random.uniform(300000, 400000)
    return x, y
...
def criar_estrelas(num_estrelas):
    ...
    y = (ESPACO_VIRTUAL_ALTURA/2)-15000
    massa = 2000000000
    raio = calcular_raio(massa)
    estrelas.append(Estrela(x, y,  -150, -150, massa, raio))
...
def criar_planetas(num_planetas):
    ...
    for _ in range(num_planetas):
        ...
        planetas.append(Planeta(x, y, 250, 250, 1000, 500))
    return planetas
...
def desenhar_estrelas():
    for estrela in estrelas:
        if estrela.ativo:
            desenhar_objeto(estrela.x, estrela.y, estrela.raio, (255, 255, 0))  # Amarelo
